...
#define cws_bucket_array_push(arr, item) ({ \
    ...
    if (arr.buckets[arr.bucket_index]->occupied<UINT_MAX) { \
        for(u32 _arr##counter_ = 0; _arr##counter_ < BUCKET_SIZE; ++_arr##counter_) { \
            if(!((arr.buckets[arr.bucket_index]->occupied>>_arr##counter_)&1)){\
...
#define cws_bucket_array_occupied(arr,index)((arr.buckets[index/BUCKET_SIZE]->occupied>>(index%BUCKET_SIZE))&1)
